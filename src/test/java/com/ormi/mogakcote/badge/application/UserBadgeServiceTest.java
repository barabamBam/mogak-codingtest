...
class UserBadgeServiceTest {
  ...
  void makeUserBadge1() {
    ...
    when(badgeRepository.findByName("COMMENT_20")).thenReturn(Optional.ofNullable(badge));

    when(userBadgeRepository.findAllByUserId(1L)).thenReturn(userBadgeList);
  ...
  void makeUserBadge2() {
    ...
    when(badgeRepository.findByName("COMMENT_50")).thenReturn(Optional.ofNullable(badge2));
