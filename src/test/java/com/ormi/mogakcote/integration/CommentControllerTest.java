...
public class CommentControllerTest {
    ...
    void setUp() {
        ...
        commentResponse = commentService.createComment(new AuthUser(user.getId()), postResponse.getId(), commentRequest);
    }

    @Test
    void testCreateComment_ok() throws Exception {
        mockMvc.perform(post("/api/v1/posts/1/comments")
                ...
                .andExpect(jsonPath("$.data.comment").value(commentRequest.getContent()));
    }

    @Test
    void testCreateComment_forbidden() throws Exception {
        mockMvc.perform(post("/api/v1/posts/1/comments")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(commentRequest)))
    ...
    @Test
    void testGetComment_ok() throws Exception {
        mockMvc.perform(get("/api/v1/posts/1/comments/" + commentResponse.getId())
                        .header("Authorization", "Bearer " + accessToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.id").value(commentResponse.getId()))
                .andExpect(jsonPath("$.data.comment").value(commentResponse.getComment()));
    }

    @Test
    void testUpdateComment_ok() throws Exception {
        CommentRequest updateRequest = new CommentRequest();
        updateRequest.setContent("Updated Comment");

        mockMvc.perform(put("/api/v1/posts/1/comments/" + commentResponse.getId())
                        .header("Authorization", "Bearer " + accessToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(updateRequest)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.id").value(commentResponse.getId()))
                .andExpect(jsonPath("$.data.comment").value(updateRequest.getContent()));
    }

    @Test
    void testUpdateComment_forbidden() throws Exception {
        mockMvc.perform(put("/api/v1/posts/1/comments/" + commentResponse.getId())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(commentRequest)))
                .andExpect(status().isForbidden());
    ...
    @Test
    void testDeleteComment_ok() throws Exception {
        mockMvc.perform(delete("/api/v1/posts/1/comments/" + commentResponse.getId())
                        .header("Authorization", "Bearer " + accessToken))
                .andExpect(status().isOk());
    }

    @Test
    void testDeleteComment_forbidden() throws Exception {
        mockMvc.perform(delete("/api/v1/posts/1/comments/" + commentResponse.getId()))
                .andExpect(status().isForbidden());
    }

    @Test
    void testGetCommentList_ok() throws Exception {
        mockMvc.perform(get("/api/v1/posts/1/comments/list")
                        .header("Authorization", "Bearer " + accessToken))
                .andExpect(status().isOk())
