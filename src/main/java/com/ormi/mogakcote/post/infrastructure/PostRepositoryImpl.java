...
public class PostRepositoryImpl implements PostRepositoryCustom {
	...
	@Override
	public Page<PostSearchResponse> searchPosts(AuthUser user, PostSearchRequest postSearchRequest, Pageable pageable) {

		String reportAlgorithms =
			algorithmRepository.findNameById(problemReportRepository.findAlgorithmIdById(post.id);
		String codeAlgorithms =
			algorithmRepository.findNameById(postAlgorithmRepository.findAlgorithmIdById(post.id);

		List<String> algorithms = new ArrayList<>();
		algorithms.add(reportAlgorithms);
		algorithms.add(codeAlgorithms);
		...
		JPAQuery<Post> query =
			jpaQueryFactory
				...
				.where(
					(post.postFlag.isPublic.eq(true).or(post.userId.eq(user.getId())))
						...
						.and(StringUtils.hasText(postSearchRequest.getAlgorithm()) ?
							(isEqual(postSearchRequest.getAlgorithm(), reportAlgorithms)
								.or(isEqual((postSearchRequest.getAlgorithm()), codeAlgorithms))) : null)
						.and(isEqual(postSearchRequest.getLanguage(), languageRepository.findNameById(post.languageId)))
						.and(postSearchRequest.isCheckSuccess() ? post.postFlag.isSuccess.eq(true) : null))
				.groupBy(post.id)
